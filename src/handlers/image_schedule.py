...
async def get_image_schedule_for_current_week(callback_query: types.CallbackQuery):
    ...
    await callback_query.bot.send_message(text="Sending image...", chat_id=callback_query.from_user.id)
    choice = callback_query.data[12:]
    if choice == "current":
        photo = await get_image_schedule_from_api(True)
    else:
        photo = await get_image_schedule_from_api(False)
    await callback_query.bot.send_photo(chat_id=callback_query.from_user.id, photo=photo)
